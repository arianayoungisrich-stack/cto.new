        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-16">
            ...
            <p className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">
              Trusted by 850+ local businesses
            </p>
            <p className="mt-4 text-base sm:text-lg text-slate-600 max-w-2xl mx-auto">
              ReplyAI handles over 45,000 conversations every month for businesses like yours. Here's what our customers are saying.
            </p>
          </div>

          {/* Stats Bar */}
          <div className="max-w-4xl mx-auto rounded-2xl bg-indigo-600 px-6 sm:px-12 py-10">
            <dl className="grid grid-cols-2 gap-8 text-center sm:grid-cols-4">
              {[
                { label: "Conversations", value: "45K+", sub: "per month" },
                { label: "Response Time", value: "<3s", sub: "average" },
                { label: "Leads Captured", value: "12K+", sub: "per month" },
                { label: "Uptime", value: "99.9%", sub: "guaranteed" },
              ].map((s) => (
                <div key={s.label}>
                  <dt className="text-sm font-semibold text-indigo-200">{s.label}</dt>
                  <dd className="mt-2 text-3xl font-bold text-white">{s.value}</dd>
                  <dd className="mt-1 text-sm text-indigo-200">{s.sub}</dd>
                </div>
              ))}
            </dl>
          </div>
          ...
          <div className="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { name: "Maria Gonzalez", role: "Owner", business: "Sunrise Dental", industry: "Dentist Office", icon: "🦷", quote: "We used to lose 3-4 new patient calls a day because we couldn't answer during procedures. ReplyAI now handles every single one — and books them straight into our calendar. We've added $8,400/month in new patient revenue.", stat1: "↑ 47% new patients", stat2: "2.1s avg response" },
              { name: "David Chen", role: "Co-Owner", business: "FlowRight Plumbing", industry: "Plumbing Company", icon: "🔧", quote: "Before ReplyAI, we'd miss 30% of after-hours emergency calls. Now every call gets an instant text back and the AI qualifies whether it's urgent. Our emergency dispatch rate went through the roof.", stat1: "↑ 62% after-hours booked", stat2: "92% lead capture rate" },
              { name: "James Wilson", role: "General Manager", business: "CoolAir HVAC", industry: "HVAC Company", icon: "❄️", quote: "ReplyAI paid for itself in the first week. During the summer heatwave, it handled 200+ inquiries in one day while our office staff focused on dispatching techs. Zero leads were dropped.", stat1: "200+ leads in one day", stat2: "$24K revenue recovered" },
              { name: "Priya Patel", role: "Founder", business: "Glow Aesthetics", industry: "Med Spa", icon: "✨", quote: "Our clients love that they can text us at midnight and get an instant response about Botox pricing, availability, or aftercare. It feels personal and our booking rate from chat is incredible.", stat1: "↑ 38% booking rate", stat2: "4.9 ★ Google rating" },
            ].map((t) => (
              <div key={t.name} className="flex flex-col rounded-2xl bg-slate-50 p-6 sm:p-8 border border-slate-100">
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white text-2xl shadow-sm border">{t.icon}</div>
                  <div>
                    <p className="text-sm font-semibold text-slate-900">{t.business}</p>
                    <p className="text-xs text-slate-500">{t.industry}</p>
                  </div>
                </div>
                <blockquote className="mt-5 text-sm leading-relaxed text-slate-600">
                  &ldquo;{t.quote}&rdquo;
                </blockquote>
                ...
                <div className="mt-3 flex gap-4 border-t pt-3 text-xs text-slate-500">
                  <span className="font-semibold text-indigo-600">{t.stat1}</span>
                  <span className="font-semibold text-emerald-600">{t.stat2}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-12 text-center">
            <p className="text-sm text-slate-500">Join 850+ businesses already using ReplyAI to never miss a lead</p>
            <div className="mt-4 flex items-center justify-center gap-x-2">
              <div className="flex -space-x-2">
                {["🏥", "🔧", "❄️", "✨", "🦷", "🏠"].map((e, i) => (
                  <div key={i} className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-sm shadow ring-2 ring-white">{e}</div>
                ))}
              </div>
              <span className="text-sm text-slate-500">+ many more</span>
            </div>
          </div>
        </div>
      </section>
